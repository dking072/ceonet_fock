...
class MaskedFockLoss(nn.Module):
    """
    MSE + MAE loss over valid AO entries, gated by a block mask.
    ...
    The L1 term keeps gradients healthy on the many near-zero off-diagonal
    entries and dominates late training; this MSE+MAE combination is the
    standard objective for Fock-block regression (QHNet, PhiSNet, DeepH).
    """
    ...
    def forward(self, pred: Dict[str, torch.Tensor],
        ...
        mse  = (diff ** 2).mean()
        mae  = diff.abs().mean()
        return self.loss_weight * (mse + self.mae_weight * mae)
